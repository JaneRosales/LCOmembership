...
from lcommsapp.forms import Membership, MembershipMeta, UserRegistration, UpdateProfile, UpdateProfileMeta, UpdateProfileAvatar, SaveCategory, SavePost, AddAvatar

category_list = Category.objects.exclude(status=2).all()
context = {
    'page_title': 'CSULCOMMS',
...
def submit_reg(request):
    context['page_title'] = "Membership Registration"
    user = User.objects.get(id=request.user.id)
    profile = UserProfile.objects.get(user=user)
    context['userData'] = user
    context['userProfile'] = profile
    if request.method == 'POST':
        ...
        # if data['password1'] == '':
        # data['password1'] = '123'
        form = Membership(data, instance=user)
        if form.is_valid():
            form.save()
            form2 = MembershipMeta(data, instance=profile)
            if form2.is_valid():
                form2.save()
                messages.success(
                    request, "Your Membership Registration is submitted successfully")
                return redirect("submit-reg")
            else:
                # form = UpdateProfile(instance=user)
                context['form2'] = form2
        else:
            context['form1'] = form
            form = Membership(instance=request.user)
